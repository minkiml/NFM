...
class LFT_block(nn.Module):
    ...
    def forward(self, x, temporal_loc = None):
        ...
        x_freq = self.hyper_vars_LFT.DFT_(x)
        # Frequency extension operationg (M)      
        _, f_tokens = self.generate_frequency_token(B=B, dev = x_freq.device, temporal_loc = temporal_loc)
        if self.lft:
            # scaling
    ...
    def generate_frequency_token(self, B, dev = None, temporal_loc = None):
        if self.lft:
            base_signal = self.norm_out((self.FT_generator(tc = temporal_loc, 
                                                           B = 1, 
                                                           L = self.hyper_vars_LFT.L_span, dev = dev)).squeeze(0).expand(B,-1,-1))
