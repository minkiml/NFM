...
class NormalizationLayer(nn.Module):
    ...
    def __init__(self, norm, hidden, affine = True, 
        ...
        if norm == "LayerNorm":
            ...
            self.norm_ = InstanceNorm_(hidden, affine = affine, var = var, mean = mean)
        else:
            self.norm_ = None
        if dc_removal:
            self.norm_2 = InstanceNorm_(hidden, affine = False, var = False, mean = True)
...
class InstanceNorm_(nn.Module):
    "Layernorm over sequence length dim"
    def __init__(self, features, affine = True,
                 eps=1e-6,  var = True, mean = True):
        super(InstanceNorm_, self).__init__()
        self.affine = affine
